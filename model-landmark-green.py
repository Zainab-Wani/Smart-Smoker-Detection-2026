...
# ============================================================
# SINGLE COLOR LANDMARK CONFIGURATION (Neon Green)
# ============================================================
# Change the BGR values below if you prefer a different single color
LANDMARK_COLOR = (0, 255, 0) 

single_color_spec = mp_drawing.DrawingSpec(
...
def start_detection():
    ...
    while camera_running:
        ...
        if frame_counter % frame_skip == 0:
            results = model(frame, imgsz=960, conf=0.60, iou=0.4)
            for r in results:
                for box in r.boxes:
        ...
        # ====================================================
        # MEDIAPIPE DETECTION WITH UNIFORM SINGLE COLOR
        # ====================================================
        ...
        # --- DRAW HAND LANDMARKS (Single Color) ---
        if hand_results.multi_hand_landmarks:
            for hand_landmarks in hand_results.multi_hand_landmarks:
                ...
                # Proximity Logic (Index Finger Tip)
                ...
                for (x1, y1, x2, y2) in smoking_boxes:
                    obj_cx = int((x1 + x2) / 2)
                    obj_cy = int((y1 + y2) / 2)
                    dist_hand_obj = math.sqrt((fx - obj_cx) ** 2 + (fy - obj_cy) ** 2)

                    if dist_hand_obj < 80:
                        hand_near_object = True
                        cv2.putText(frame, "HAND HOLDING OBJECT", (30, 60),
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)

                        root.after(0, lambda: hand_label.config(
                            text="✋ Hand Holding Vape/Cigarette", fg="red"
                        ))
        else:
            root.after(0, lambda: hand_label.config(
                text="✋ Hand Status: Normal", fg="#facc15"
            ))

        # --- DRAW FACE MESH & MOUTH REGION (Single Color) ---
        if face_results.multi_face_landmarks:
            for face_landmarks in face_results.multi_face_landmarks:
                # 1. Render Face Mesh Contours using the unified spec
                mp_drawing.draw_landmarks(
                    image=frame,
                    landmark_list=face_landmarks,
                    connections=mp_face_mesh.FACEMESH_CONTOURS,
                    landmark_drawing_spec=None,
                    connection_drawing_spec=single_color_spec
                )

                # 2. Extract and draw specific boundaries over the Mouth Region
                mouth_indices = [
                    61, 146, 91, 181, 84, 17, 314, 405, 321, 375, 291, 308,
                    324, 318, 402, 317, 14, 87, 178, 95, 88, 178
                ]
                mouth_points = []
                ...
                if mouth_points:
                    pts = np.array(mouth_points, np.int32).reshape((-1, 1, 2))
                    # Render the mouth polygon boundary in the same unified color
                    cv2.polylines(frame, [pts], True, LANDMARK_COLOR, 2)

                # Calculate mouth center tracking point
                mouth_landmarks_indices = [61, 291, 0, 17, 78, 308]
                mouth_coords = []
                for idx in mouth_landmarks_indices:
                    lm = face_landmarks.landmark[idx]
                    mouth_coords.append((int(lm.x * w), int(lm.y * h)))

                if mouth_coords and smoking_boxes:
                    mouth_cx = sum([p[0] for p in mouth_coords]) // len(mouth_coords)
                    mouth_cy = sum([p[1] for p in mouth_coords]) // len(mouth_coords)

                    # Center point tracker dot in the same color
                    cv2.circle(frame, (mouth_cx, mouth_cy), 5, LANDMARK_COLOR, -1)

                    for (x1, y1, x2, y2) in smoking_boxes:
                        obj_cx = int((x1 + x2) / 2)
                        obj_cy = int((y1 + y2) / 2)
                        dist_obj_mouth = math.sqrt((obj_cx - mouth_cx) ** 2 + (obj_cy - mouth_cy) ** 2)

                        if dist_obj_mouth < 100:
                            object_near_mouth = True
                            cv2.putText(frame, "OBJECT NEAR MOUTH", (30, 100),
        ...
        # FINAL ALERT LOGIC
        # ====================================================
        if person_detected and smoking_object_detected and hand_near_object and object_near_mouth:
            root.after(0, lambda: status_box.config(text="ALERT", bg="#dc2626"))
            ...
            if time.time() - last_alert_time > 2:
                ...
                root.after(0, lambda: counter_label.config(text=f"⚠ Alerts: {alert_count}"))
        else:
            root.after(0, lambda: status_box.config(text="SAFE", bg="#16a34a"))

        # Object Label Update
        if detected_objects:
            object_text = ", ".join(list(set(detected_objects)))
        else:
            object_text = "None"
        root.after(0, lambda: object_label.config(text=f"📦 Objects: {object_text}"))

        # Display Frame in Tkinter Canvas
        img = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        img = Image.fromarray(img).resize((950, 720))
